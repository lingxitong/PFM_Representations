...
def test_linear_probe(
    ...
    # Always return full probability matrix [batch_size, num_classes]
    probs_all = linear_classifier.predict_proba(test_feats).detach().cpu().numpy()
    
    # Set roc_kwargs for get_eval_metrics (this function may need special handling for binary classification)
    if NUM_C == 2:
        roc_kwargs = {}
        # For compatibility with get_eval_metrics, if it needs 1D probability for binary classification
        probs_for_metrics = probs_all[:, 1]
    else:
        roc_kwargs = {"multi_class": "ovo", "average": "macro"}
        probs_for_metrics = probs_all
    ...
    eval_metrics = get_eval_metrics(targets_all, preds_all, probs_for_metrics, True, prefix, roc_kwargs)
...
def _fit_logreg(
    ...
    """
    if use_sklearn:
        classifier = sk_LogisticRegression(C=cost, max_iter=max_iter, verbose=verbose, random_state=random_state, device=device)
    else:
        classifier = LogisticRegression(C=cost, max_iter=max_iter, verbose=verbose, random_state=random_state, device=device)
